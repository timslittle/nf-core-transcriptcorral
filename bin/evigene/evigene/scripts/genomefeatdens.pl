    ...
    if(1) { # unless($ov) 
      ...
      ## OPTION: process @sc2 other scores, but need option of type list for those.
        if(@MORESCORES and @sc2) {
          for(my $j=0; $j < @sc2; $j++) { 
            my $ktype=$MORESCORES[$j]; next if($ktype=~/^skip|^\!|^no\./i);
            my $kval=$sc2[$j]; 
...
sub readBed1
{
  ...
  while(<>){
    next unless(/^\w/);
    my($r,$tb,$score,@sc2)=split"\t";  #only: scaffold_1      40      6
    next if($tb<1);
    ...
    #upd20jul: add morescores to bed1 type
    if(@MORESCORES and @sc2) {
      for(my $j=0; $j < @sc2; $j++) { 
        my $ktype=$MORESCORES[$j]; next if($ktype=~/^skip|^\!|^no\./i);
        my $kval=$sc2[$j]; 
        $nc{$r}{$bn}{$ktype} += $kval;   # equiv to ctype += score
        $tp{$ktype}++; # above?
      }
    }
