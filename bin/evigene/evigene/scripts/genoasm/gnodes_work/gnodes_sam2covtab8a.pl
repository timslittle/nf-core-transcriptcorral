...
sub MAIN_stub {}
  ...
  my %generdid=(); #UPD21JU, mem overflow problem with huge drospse SRR read set (CDepth=160 )
  ...
  if($cdstab and ($ok,$inh)= openRead($cdstab) and $ok) { 
    ...
    while(<$inh>){ 
      ...
      if($hasgeneids == 0) { # look for col, probly last
        ...
        $hasgeneids= -1 if($hasgeneids == 0); # stop looking
      } elsif($hasgeneids>0) {
  ...
  if( GENECOVBINS >= 4) {
  putGenetab8e($outgenepart) if($SAVE_GENECOV); # simplified, merge-able
  } elsif( GENECOVBINS == 3) {
...
sub mergeparts {
  ...
  my $mgeneids= (defined $outgenetab and $SAVE_RDIDS)?1:0; # GENECOVBINS >= 1
    ...
    for my $cb (sort{ $a <=> $b } keys %{$tab{$id}} ) {
      ...
      if($mgeneids) {
        ...
        if($gids =~ /\w\w/){ my %gids= map{ $_ => 1 } split ",",$gids;  
          $gids=join",",sort keys %gids; push @vals,$gids; 
          }
      }
...
sub putShortmap { 
  ...
  my ($cdsread,$geneids)=  (0,0);  
  if($NCDS){ my($idp,$idn)= readIdNum($rdid);
    $cdsread= $cdsrdid{$idp}[$idn]||0; 
    if($SAVE_GENECOV and my $gids= $generdid{$idp}{$idn}){ my @gids= split",",$gids; $geneids= \@gids; }
  } 
