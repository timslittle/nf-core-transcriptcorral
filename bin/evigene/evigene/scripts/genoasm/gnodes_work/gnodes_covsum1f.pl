...
# my $CKCONTAM= 0; #? flag has contam annots, set in readAnnots 
if($anntable) {
...
sub MAIN_loop {}
my($lcr,$lcb,$lucg,@cbin,@rdlen)=(0,0,0);
...
sub rangeKuni {
  ...
  {  # test, replace w/ kbu_med if it seems accurate: it *might be* bogus
    $kumin= ($kulo == 0) ? $kuhi * 0.80 : $kulo * 0.80;
    $kumax= ($kuhi == 0) ? $kulo * 1.20 : $kuhi * 1.10;
...
sub pickKuni {
  ...
  my($gucg_n, $gucg_med, $gucg_ave, $gucg_err)= (0) x 4; # genexcopy
  my($kumin,$kumax)=(0,999999); # use when ku is set in metadata

  my $kulo= $samvals{$asmid}{kulo}|| 0; # int or real val
  my $kuhi= $samvals{$asmid}{kuhi}|| 0;
  if($kulo and $kuhi) { ($kulo,$kuhi)= ($kuhi,$kulo) if($kulo > $kuhi); }
  my $kutype=($kulo or $kuhi)?"set":"";
  if($kulo or $kuhi){ $kuept{'KUset'} = [$kulo,$kuhi,1]; } #???
  ...
  if(UPD21JUN and $genexcopy) {
    ...
    if($gucg_n and $gucg_med) {
      $kuept{'CMgenes'} = [$gucg_med,$gucg_ave,$gucg_n]; 
      if(1) {
...
unless(UPD21JUN) { # dont set kulo/hi yet    
  ...
  if(UCG_KU_SPECIAL and UPD21JUN) { # add KU_ucg_cov to kuept
    ...
    $kuept{'KUucg'} = [$kucg_med,sprintf("%.2f",$kucg_ave),$kucg_n];
...
#     }
  ...
  if(UPD7f or UPD21JUN) {
    ...
    $kuept{'CDSbus'} = [$kbu_med,$kbu_ave,$kbu_n] if($kbu_n >= $MINN);
    $kuept{'uniqasm'}= [$kuasm_med,$kuasm_ave,$kuasm_n] if($kuasm_n >= $MINN);
              
    for my $pt (@ptadd) {
      if(my $pv= $sum{$pt}{'rdacovm'}) { 
        my($pmed,$pave,$pn)= @{$pv}[1,2,4];
        $kuept{$pt}= [$pmed,$pave,$pn] if($pn >= $MINN);
      }
    }
    ...
    ## BUG here: undefined value as an ARRAY reference >> caused by ? data bug?
    # my($kc2med,$kc2ave,$kc2n)= @{$kuept{$pt[1]}};
    my($kc2med,$kc2ave,$kc2n)= (@pt>1) ? @{$kuept{$pt[1]}} : (0,0,0);
    ...
    # if($debug){
    #   warn "#pickKuni bestpart: $kc_bestpart\n#Item\tCmed\tCave\tCn\tCerr\n";
    #   for my $pt (@pt){ warn join("\t","# $pt",@{$kuept{$pt}})."\n"; }
    # }
...
#       $kutype= "set_UCG";
  ...
  if($kulo == 0) {
    $kulo= _minnot0( _minnot0($kbu_med,$kbu_ave),$kuasm_med); # check kbu_ave also ?
  }
  if($kuhi == 0) {
    $kuhi= _max($kbu_med,$kuasm_med);   # ? but not kbu_ave may be hi due to dupls
...
# add range w/ KU sem=1.3, LN/C=50..54 = 150 Mb .. 162 Mb
  ...
  if($readlen and $kuhi) {
    ...
    $gsen= ($twoku) ? "$gsf_lnc-$gse_lnc" : $gse_lnc;  $gsen .=" Mb";
    $gsem= ($twoku) ? "$gsf_lmc-$gse_lmc" : $gse_lmc;  $gsem .=" Mb"; # FIXMEd: gse,gsf swapt for Nmaprd
  ...
  if(UPD21AUG and defined $sum{'CONTAM'}) {
    ...
    $contamb= sprintf "%.0f",  $mbCBIN * $mv->[4]||0;
  }
