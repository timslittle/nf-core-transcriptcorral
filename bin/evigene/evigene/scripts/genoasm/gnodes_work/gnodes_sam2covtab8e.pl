...
=cut
  ...
  ($haslen)= readChrtab($inchrtab, $inbam); # ret=$haslen
  ...
  # need to rewrite to move cds-te-readid info into chr.bam rows, could also slim down readid hash w/ int ids
  # .. move this call AFTER read_idclass, readChrtab  (failing samtools view -H there w/ big-pig)
  ($NCDS)= readReadIds($cdstab); # ret=$NCDS
  #----------------
...
sub readReadIds { 
  ...
  if($cdstab and ($ok,$inh)= openRead($cdstab) and $ok) { 
    ...
    while(<$inh>){ 
      ...
      # if(UPD21AUG) { $redata[$idn]= "$crclass\t$geneids"; }
...
sub readFilter {  
  ...
  while(<$inhand>) {
    ...
    if($badmap>0) { 
      putBadmap($badmap, $rdid, $inrdlen, [$cr,$cb,$cig,0,$fl] );
...
sub putBadmap {
  my($badmaptype, $rdid, $rdlen, $saveset)=@_;
