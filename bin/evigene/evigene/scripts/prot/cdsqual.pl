...
use constant VERSION => 'beta:2019.08.28'; #  UPD19a here
...
use constant UPD19a => 1;
my $noFickett= $ENV{noCF} || 0;
my $noHexamer= $ENV{noCH} || 0; $noHexamer=0 if($noFickett);
...
sub makeCdsQual {
  ...
  sub puta { 
    ...
    #UPD19a: opt use only 1 of fickett or hexamer score, set other eq chosen for now..
    my($fval,$fclass,$fn)=(0,'Unknown',0);
    my($hval,$hclass,$hn)=(0,'Unknown',0);
    if(UPD19a){ 
      ($fval,$fclass,$fn)= ($noFickett) ? (0,0) : iscode_fickett($id,$seq); #fail now, 000 only 
      ($hval,$hclass,$hn)= ($noHexamer) ? (0,0) : iscode_hexamer($id,$seq);
      if($noFickett) { ($fval,$fclass,$fn)=($hval,$hclass,$hn); }
      if($noHexamer) { ($hval,$hclass,$hn)=($fval,$fclass,$fn); }
    } else {
      ($fval,$fclass,$fn)= iscode_fickett($id,$seq); 
      ($hval,$hclass,$hn)= iscode_hexamer($id,$seq);
    }
    ...
    if(UPD19a){ 
      my @cp=($cpclass);
      push @cp, "CF:$fclass/$fval" unless($noFickett);
      push @cp, "CH:$hclass/$hval" unless($noHexamer);
      push @cp, "CA:$aaclass"; #unless?
      push @val, join(",",@cp);
    } else {
