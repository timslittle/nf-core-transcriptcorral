...
my $debug= 0;
use constant VERSION => '2019.12.03'; # UPD1912 small fixes + pHETERO adjust/lower all identity params by 1-3% TEST
...
die "usage:  asmrna_dupfilter.pl -aasize=name.aa.count -trsize=name.tr.count 
 ...
 opts: -pCDSOK=$OK_CDSUTR percents  -ablastab=traa-refaa-blast.table  .. more options, see source.
...
=item UPD1908 classifytr drop valid alts problem
  
    drops of 'true' reference genes, human, plant, are mainly 'althi' with 2 pflag qualities:
      aadup (>= 99% ident from cdhit.aa cluster)
      utrpoor/bad
    large portion have pia = 100/100/ but signif num have 100/99..80..small %align-global, should treat those as valids?  
    maybe drop aadup criteria, or at least balance it w/ other rescue criteria: pi/pa < 100/100 ? 
    add criteria of total non-ident cds-bases? some threshold (10? 100?) keeps alt?

    pflag, see above constants 2=utrbad 4=utrpoor 8=aadup
    see also FIXME 201402 drop.mains problem
      AAPART cutoff is problem, with UTRBAD/POOR, causing drop.mains of uniq orthologs 
      AAPART & AAMIN, AAMINBAD, AAMINPOO  need fix to prevent drop uniq/valid main/noclass genes, 
  
    drop.parthi has similar loss of valid ref alts, adjust parthi criteria?
      frag/part == $tsize < $ALTFRAG*$qsize; ALTFRAG = 0.50 default, should retest
      
    drop valid alts via perffrag: this happens when true alt is perfect substring of longer alt,
      not sure this is likely alternating-exon gene structure, but may be,
      more likely is experimentally found alternate start site in same exon structure.
      
    for ref_arath16ap.trclass
      5167 drop.althi, 3695 have aadup, 1445/1472 have utrpoor/bad of no-aadup
      5601 okay.althi, 0 have aadup, 1062 have utrpoor/bad

    for bevgc_evg3weed1rd.trclass
      85208 drop.althi, 41846 have aadup, 30681/43362 have utrpoor/bad, 14032/43362 aapartial of no-aadup
      106526 okay.althi, 0 have aadup, 32274 have utrpoorbad, 22851 aapartial

    for ref_human.trclass
      26715 drop.althi, 14346 aadup, 12305/12369 have utrpoor/bad of no-aadup
      22563 okay.althi, 0 aadup, 5975 have utrpoor/bad
    
    for trev19human.trclass
      126070 drop.althi, 45301 aadup, 37126/80769 utrpoorbad of noaadup, 35811/80769 aapartial of noaadup
      138448 okay.althi, 0 aadup, 39146 utrpoorbad, 30744 aapartial
...
if(CODEPOT1607) {
  $tcode= $sizeval{$tid}{'codepot'}||""; # $tcode =~ /^Noncode/ set what? kAATINY+kAAUTRBAD
  if($tcode and $tcode =~ /^Noncode/) { # /^N/ enough
  $tcode=0 if($tbscore>0);
  # $tcode=0 if($aw > $AAPART);# UPD1908 TESTING,not very accurate but use to separate too many tiny aa
  $tcode=0 if($aw > 139 or ($aw > 99 and not $ispoor)); #?? which? many utrorf/utrbad for aa>99
  ...
  if(UPD1908) { 
    # ?? need finer filter for althi than current ($aamainpi > $AADUP_IDENT)
    # bump AADUP_IDENT from 98 to 99?  needs testing
    # losing too many valid alts, but still have too many invalids
  }
  if($aamainpi > $AADUP_IDENT) { # TEST aacluster added info on dup prots; AADUP_IDENT=98 default
  ...
  if($cla =~ /parthi|frag0aa/) {
    $keepdrop.= "drop";
    # not frag? if($ispoor & kNONCODE) { $cla= $cla."nc"; $keepdrop.="okay"; } 
    
  } elsif($cla =~ /althi/) {
if(UPD1908) {  
    # valid althi loss: maybe bump ($pali > 75) to ($pali > 95) ? needs test
    ...
    # UPD1908 maybe modify aacons rescue for poor-noclass, as with parthi/frag
    if(UPD1908) { if($ispoor == kNONCODE) { $cla= $cla."nc"; $keepdrop.="okay"; } } # dont rescue NC noclass if other ispoor
    $keepdrop.= ($ispoor)?"drop":"okay"; #? dont drop? ditto to altmid
    
  } else { # other altmid/low 
    ## code err here, in $cla?: Can't call method "other" without a package or object reference
    ##? if(UPD1908) { if($ispoor == kNONCODE) { $cla= "$cla"."nc"; $keepdrop.="okay"; } } dont rescue NC noclass if other
    $keepdrop.= ($ispoor)?"drop":"okay"; #? dont drop this class, ispoor maybe uniq prot: maybeok?
  }
...
=item identityclass
  ...
  ## UPD1912: evigene/scripts/rnaseq/asmrna_identityclass4a.pl
